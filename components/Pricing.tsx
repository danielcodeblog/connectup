...
const DEFAULT_PLANS: PricingPlan[] = [
  {
    ...
    isPopular: false,
    features: [
      { text: "1 Pitch / Startup Listing", included: true },
      ...
      { text: "Up to 2 Pitches (Pro Plan required)", included: false },
  ...
  },
  {
    ...
    isPopular: true,
    features: [
      { text: "Up to 2 Pitches / Startup Listings", included: true },
      { text: "Standard Profile Listing", included: true },
  ...
  const handleToggle = (checked: boolean) => {
    ...
    if (checked) {
      confetti({
        ...
        spread: 60,
        origin: { y: 0.6 },
        colors: ["#FFBF00", "#CFB53B", "#E0B0FF", "#ffffff"],
        ticks: 150,
        gravity: 1.1,
        decay: 0.95,
        startVelocity: 25,
    ...
    <div className="container mx-auto py-24 sm:py-32 px-4 sm:px-6 relative z-10 max-w-[1400px] w-full overflow-hidden md:overflow-visible text-zinc-900 bg-white">
      ...
      <div className="text-center space-y-4 mb-16">
        <h2 className="text-5xl sm:text-7xl md:text-8xl font-display font-black text-center tracking-tighter uppercase mb-8">
          <span className="text-zinc-950">Flexible</span> <span className="text-[#FACC15]">Pricing</span>
        </h2>
          ...
          >
            {isMonthly && (
              <motion.span
                ...
                className="absolute inset-0 bg-[#FFBF00] rounded-full -z-10"
      ...
      <div className="w-full overflow-hidden lg:overflow-visible py-8 flex justify-center">
        <div ref={containerRef} className="relative flex flex-row overflow-x-auto snap-x snap-mandatory gap-6 w-full pt-4 pb-8 px-4 justify-start sm:justify-center lg:grid lg:grid-cols-3 lg:gap-8 xl:gap-10 lg:w-full max-w-7xl lg:px-4 lg:pb-0 scrollbar-none [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  plan.isPopular 
                    ? "bg-[#FFBF00] text-zinc-950 border-amber-300 z-10 shadow-xl" 
                    : "bg-black border-zinc-800 hover:border-zinc-700 z-0 text-white shadow-xl"

                )}
              >
                {/* Popular Pill */}
                {plan.isPopular && (
                  <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-zinc-950 text-[#FFBF00] px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-1 z-20">
              ...
              <div className={cn(
                ...
                <h3 className={cn(
                  ...
                  plan.isPopular ? "text-zinc-950 text-center font-semibold" : "text-white"
                )}>
                  {plan.name}
                </h3>
                ...
                <p className={cn(
                  ...
                  plan.isPopular ? "text-zinc-800 text-center max-w-xs mx-auto" : "text-zinc-400"
                )}>
                  {plan.description}
                </p>
                ...
                <div className={cn(
                  ...
                  plan.isPopular ? "border-zinc-950/10 justify-center w-full" : "border-zinc-800 w-full"
                )}>
                  {isCustom ? (
                    <span className={cn(
                      "text-3xl sm:text-4xl font-black tracking-tight uppercase font-sans",
                      plan.isPopular ? "text-zinc-950" : "text-white"
                    )}>
                      {plan.price}
                    </span>
                  ) : (
                    <>
                      ...
                      <span className={plan.isPopular ? "text-zinc-800 font-medium text-xs sm:text-sm" : "text-zinc-400 font-medium text-xs sm:text-sm"}>
                ...
                {!isMonthly && !isCustom && planPriceValue > 0 && (
                  <p className={cn("text-xs font-semibold mb-6", plan.isPopular ? "text-zinc-900 text-center" : "text-[#FFBF00]")}>
                ...
                <ul className={cn("space-y-4 flex-1 mb-4 w-full", plan.isPopular ? "max-w-xs mx-auto text-left" : "")}>
                  {plan.features.map((feature, idx) => (
                    <li key={idx} className="flex items-start gap-3 text-xs sm:text-sm font-light">
                      {feature.included ? (
                        <div className={plan.isPopular ? "text-zinc-950 mt-1 flex-shrink-0" : "text-[#FFBF00] mt-1 flex-shrink-0"}>
                           <Check className="h-4 w-4 stroke-[3]" />
                        </div>
                      ) : (
                        <div className={plan.isPopular ? "text-zinc-700 mt-1 flex-shrink-0" : "text-zinc-600 mt-1 flex-shrink-0"}>
                      ...
                      <span className={cn(
                        ...
                        feature.included 
                          ? (plan.isPopular ? "text-zinc-900" : "text-zinc-200") 
                          : (plan.isPopular ? "text-zinc-800/60 line-through decoration-zinc-950/20" : "text-zinc-600 line-through decoration-zinc-800")
                      )}>
                        {feature.text}
                      </span>
                    </li>
