...
import { Loading03Icon, Cancel01Icon, Upload01Icon, PencilEdit02Icon, DashboardSquare01Icon, Activity02Icon, Video01Icon, PlusSignIcon, Delete02Icon } from 'hugeicons-react';
...
export const PitchEditor: React.FC<PitchEditorProps> = ({ startup, onSave, onCancel }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      await onSave(formData as Startup);
    } catch (error) {
  ...
  const handleAddFundingRound = () => {
    const newRound = { id: crypto.randomUUID(), round: 'Seed', amount: '', date: '', investor: '' };
    setFormData({...formData, fundingHistory: [...(formData.fundingHistory || []), newRound]});
  };

  const handleUpdateFundingRound = (id: string, field: string, value: string) => {
    setFormData({
        ...formData,
        fundingHistory: formData.fundingHistory?.map(r => r.id === id ? { ...r, [field]: value } : r)
    });
  };

  const handleRemoveFundingRound = (id: string) => {
    setFormData({
        ...formData,
        fundingHistory: formData.fundingHistory?.filter(r => r.id !== id)
    });
  };
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 sm:px-8 py-6 sm:py-8 space-y-8 no-scrollbar">
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-1.5">
              <label className="text-[9px] font-black text-zinc-500 uppercase tracking-widest ml-1">Social Media URL</label>
              <input 
                type="url" 
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <h3 className="text-xs font-black text-zinc-400 uppercase tracking-widest flex items-center gap-2">
                <IconWrapper icon={Activity02Icon} size={14} /> Funding History
              </h3>
              <button 
                type="button" 
                onClick={handleAddFundingRound}
                className="flex items-center gap-1 text-xs font-bold text-brand-primary hover:text-brand-primary/80 transition-colors bg-brand-primary/10 px-3 py-1.5 rounded-lg"
              >
                <IconWrapper icon={PlusSignIcon} size={14} /> Add Round
              </button>
            </div>
            
            <div className="space-y-4">
              {(!formData.fundingHistory || formData.fundingHistory.length === 0) ? (
                <div className="text-center py-6 border border-zinc-800 border-dashed rounded-2xl bg-zinc-950/50">
                  <p className="text-sm font-medium text-zinc-500">No funding history added yet.</p>
                </div>
              ) : (
                formData.fundingHistory.map((round) => (
                  <div key={round.id} className="p-4 sm:p-5 border border-zinc-800 rounded-2xl bg-zinc-950/50 space-y-4 relative group">
                    <button 
                      type="button"
                      onClick={() => handleRemoveFundingRound(round.id)}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        <label className="text-[9px] font-black text-zinc-500 uppercase tracking-widest ml-1">Round</label>
                        <input 
                          type="text" 
                          value={round.round} 
                          onChange={(e) => handleUpdateFundingRound(round.id, 'round', e.target.value)}
                          placeholder="e.g. Seed, Series A"
                          className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-sm font-bold text-white focus:outline-none focus:border-brand-primary transition-all"
                        />
                      </div>
                      <div className="space-y-1.5">
                        <label className="text-[9px] font-black text-zinc-500 uppercase tracking-widest ml-1">Amount</label>
                        <input 
                          type="text" 
                          value={round.amount} 
                          onChange={(e) => handleUpdateFundingRound(round.id, 'amount', e.target.value)}
                          placeholder="e.g. $1M"
                          className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-sm font-bold text-white focus:outline-none focus:border-brand-primary transition-all"
                        />
                      </div>
                      <div className="space-y-1.5">
                        <label className="text-[9px] font-black text-zinc-500 uppercase tracking-widest ml-1">Date</label>
                        <input 
                          type="text" 
                          value={round.date} 
                          onChange={(e) => handleUpdateFundingRound(round.id, 'date', e.target.value)}
                          placeholder="e.g. Oct 2023"
                          className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-sm font-bold text-white focus:outline-none focus:border-brand-primary transition-all"
                        />
                      </div>
                      <div className="space-y-1.5">
                        <label className="text-[9px] font-black text-zinc-500 uppercase tracking-widest ml-1">Key Investor(s)</label>
                        <input 
                          type="text" 
                          value={round.investor} 
                          onChange={(e) => handleUpdateFundingRound(round.id, 'investor', e.target.value)}
