  ...
  return (
    ...
    >
      <motion.div 
        ...
        className="bg-white w-full h-full sm:h-auto sm:max-w-lg md:max-w-xl lg:max-w-xl sm:rounded-[32px] md:rounded-[40px] shadow-2xl overflow-hidden flex flex-col relative sm:max-h-[85vh] md:max-h-[80vh]"
      >
        {/* Header */}
        <div className="px-6 sm:px-8 pt-8 pb-5 flex justify-between items-start border-b border-zinc-100">
          <div>
            <h2 className="text-3xl sm:text-4xl font-display font-black text-zinc-900 tracking-tighter leading-none mb-2">Refine Pitch</h2>
          </div>
          <button 
            onClick={onCancel}
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 sm:px-8 py-6 sm:py-8 space-y-8 no-scrollbar">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
              <div className="space-y-1.5">
                ...
                <input 
                  ...
                  className="w-full bg-white border border-zinc-100 rounded-xl px-5 py-3.5 text-sm sm:text-base font-bold text-zinc-900 focus:outline-none focus:ring-4 focus:ring-brand-primary/10 focus:border-brand-primary transition-all"
                />
              </div>
              <div className="space-y-1.5">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest ml-1">Video URL (YouTube, Vimeo, or direct link)</label>
                <input 
                  type="text" 
                ...
                <div className="flex items-center gap-2 mt-2">
                  <button 
                    ...
                    className="px-4 py-2 bg-zinc-100 hover:bg-zinc-200 rounded-xl text-xs font-bold text-zinc-900 transition-colors"
                  >
                    {isUploadingVideo ? 'Uploading...' : 'Upload Video File'}
                  </button>
                  <input type="file" ref={videoInputRef} className="hidden" accept="video/*" onChange={handleVideoUpload} />
                </div>
                <p className="text-[10px] text-zinc-400 font-medium ml-1">Paste a link to your pitch video or upload a file. We support YouTube, Vimeo, and direct video links.</p>
              </div>
            </div>
          </div>
        ...
        {/* Footer Actions */}
        <div className="px-6 sm:px-8 py-6 sm:py-7 bg-white border-t border-zinc-100 flex gap-4">
          <div 
            onClick={handleSubmit}
            className={`flex-1 h-14 sm:h-16 bg-zinc-800 text-white rounded-xl sm:rounded-2xl font-black text-base sm:text-lg hover:bg-zinc-700 transition-all active:scale-[0.98] shadow-xl flex items-center justify-center gap-3 cursor-pointer ${isSaving ? 'opacity-50 pointer-events-none' : ''}`}
          >
            {isSaving ? (
