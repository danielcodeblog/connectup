  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-4 sm:p-6 overflow-hidden min-h-[600px]">
      <div className="w-full max-w-sm sm:max-max-md relative flex flex-col items-center">
        ...
        >
          <div 
            className="h-40 sm:h-48 w-full flex items-center justify-center relative overflow-hidden"
            style={{ background: currentFounder.image }}
          >
            {currentFounder.avatarUrl ? (
              <div className="absolute inset-0 flex items-center justify-center bg-black/20 backdrop-blur-xs">
                <img 
                  src={currentFounder.avatarUrl} 
          ...
          <div className="p-6 sm:p-8">
            <div className="flex justify-between items-start mb-1">
              <h3 className="text-2xl sm:text-3xl font-black">{currentFounder.name}</h3>
              {currentFounder.mrr && (
            ...
            <div className="text-sm sm:text-base font-bold text-[#EAB308] mb-4 tracking-tight">{currentFounder.title}</div>
            
            <p className="text-white/80 font-normal mb-6 line-clamp-3 text-sm sm:text-base leading-relaxed">"{currentFounder.tagline}"</p>
            
            <div className="space-y-2.5 text-xs sm:text-sm text-white/60">
              ...
              <div className="flex justify-between pb-2 border-b border-white/5">
                <span className="flex items-center gap-2"><Users size={14}/> Team & Location</span>
                <span className="font-semibold text-white">{currentFounder.members} members • {currentFounder.location || 'Remote'}</span>
              </div>
              <div className="flex justify-between pt-2 items-center">
                <span className="font-black text-white/40 text-[10px] uppercase tracking-widest">Seeking Funding</span>
