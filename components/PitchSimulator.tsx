...
export const PitchSimulator: React.FC = () => {
  ...
  return (
    <div className="bg-white rounded-[32px] p-6 sm:p-8 shadow-2xl shadow-black/[0.02] border border-zinc-100 relative overflow-hidden group flex flex-col h-full">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 pb-6 border-b border-zinc-50 relative z-10">
        <div>
          <div className="flex items-center gap-2 mb-1">
             <h3 className="text-xl font-display font-black text-zinc-900">30s Pitch Teleprompter</h3>
          </div>
          <p className="text-zinc-400 text-[10px] font-black uppercase tracking-widest">Pro Script Assistant</p>
        </div>
      ...
      <div className="relative z-10 mb-8 flex-1 flex flex-col">
        <div className="flex items-center justify-between gap-4 mb-6">
          <div className="flex flex-col">
            <div className="flex items-center gap-2 font-display">
              ...
              <span className="text-zinc-400 text-xl font-black mt-2">/ 30.0s</span>
            </div>
          </div>
        ...
        <div className="bg-zinc-50 rounded-[32px] p-8 min-h-[260px] flex items-center relative overflow-hidden shadow-inner border border-zinc-100">
           {/* Animated Teleprompter Scanline */}
           <motion.div 
             animate={{ x: isPlaying ? ["-100%", "200%"] : "-100%" }}
             transition={{ duration: 1.5, repeat: Infinity, ease: "linear" }}
             className="absolute inset-y-0 w-32 bg-white/40 blur-2xl pointer-events-none"
           />
          ...
          {isEditing ? (
            <div className="w-full space-y-6 relative z-10">
              <div className="group">
                <span className="text-[9px] uppercase tracking-widest font-black text-zinc-400 block mb-2 group-focus-within:text-brand-primary transition-colors">Hook (0-5s)</span>
                <textarea
                  value={hookText}
                  onChange={(e) => setHookText(e.target.value)}
                  className="w-full bg-white border border-zinc-200 rounded-2xl p-4 text-zinc-900 text-sm font-medium outline-none focus:ring-2 focus:ring-brand-primary/20 h-16 resize-none transition-all"
                />
              </div>
              <div className="group">
                <span className="text-[9px] uppercase tracking-widest font-black text-zinc-400 block mb-2 group-focus-within:text-brand-primary transition-colors">Solution (5-20s)</span>
                <textarea
                  value={solutionText}
                  onChange={(e) => setSolutionText(e.target.value)}
                  className="w-full bg-white border border-zinc-200 rounded-2xl p-4 text-zinc-900 text-sm font-medium outline-none focus:ring-2 focus:ring-brand-primary/20 h-20 resize-none transition-all"
                />
              </div>
              <div className="group">
                <span className="text-[9px] uppercase tracking-widest font-black text-zinc-400 block mb-2 group-focus-within:text-brand-primary transition-colors">Ask & CTA (20-30s)</span>
                <textarea
                  value={askText}
            ...
            <div className="font-display text-2xl sm:text-3xl leading-snug tracking-tight text-zinc-200 transition-all duration-500 relative z-10 w-full">
              <div className={cn("transition-all duration-500 mb-4", currentSection === 'hook' ? "text-zinc-900 scale-105 origin-left" : "blur-[1px]")}>
                {hookText}
              </div>
              <div className={cn("transition-all duration-500 mb-4", currentSection === 'solution' ? "text-zinc-900 scale-105 origin-left" : "blur-[1px]")}>
                {solutionText}
              </div>
              <div className={cn("transition-all duration-500", currentSection === 'ask' ? "text-zinc-900 scale-105 origin-left" : "blur-[1px]")}>
                {askText}
              </div>
            </div>
          )}
        ...
        <div className="h-2 bg-zinc-100 rounded-full overflow-hidden relative mt-8">
          <div 
        ...
        <div className="flex items-center gap-3">
          {!isPlaying ? (
            <button
              onClick={handleStart}
              disabled={isEditing}
              className="h-12 px-8 rounded-xl bg-zinc-900 hover:bg-black text-white text-[11px] font-black uppercase tracking-[0.2em] flex items-center gap-3 transition-all active:scale-95 cursor-pointer disabled:opacity-50 shadow-lg"
            >
            ...
            <button
              onClick={handleStop}
              className="h-12 px-8 rounded-xl bg-brand-primary text-black text-[11px] font-black uppercase tracking-[0.2em] flex items-center gap-3 transition-all active:scale-95 cursor-pointer shadow-lg shadow-brand-primary/20"
            >
          ...
          {(currentTime > 0 || isPlaying) && (
            <button
              onClick={handleReset}
              className="h-12 w-12 rounded-xl bg-white border border-zinc-100 hover:bg-zinc-50 text-zinc-400 hover:text-zinc-900 transition-colors cursor-pointer flex items-center justify-center"
              title="Reset"
            >
