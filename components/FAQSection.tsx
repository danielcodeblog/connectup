        ...
        <section id="faq" className="w-full py-24 md:py-32 bg-white relative z-10 px-6 border-t border-zinc-100">
            <div className="max-w-4xl mx-auto">
                ...
                <div className="space-y-4">
                    {faqs.map((faq, index) => (
                        <div key={index} className="bg-black border border-zinc-800 rounded-[1.5rem] overflow-hidden transition-all duration-300 shadow-xl hover:border-zinc-700 hover:bg-zinc-950">
                            <button
                                onClick={() => setOpenIndex(openIndex === index ? null : index)}
                                className="w-full p-6 sm:p-7 flex items-center justify-between text-left transition-all hover:bg-zinc-900/60 cursor-pointer"
                            >
                                <span className="font-serif text-lg sm:text-xl font-normal text-white pr-4">{faq.question}</span>
                                <div className={`p-1.5 rounded-full border flex-shrink-0 transition-colors ${openIndex === index ? 'bg-[#FFBF00] text-zinc-950 border-transparent' : 'text-zinc-400 border-zinc-800 bg-zinc-900'}`}>
                                    {openIndex === index ? <Minus size={16} /> : <Plus size={16} />}
                                </div>
                            </button>
                            {openIndex === index && (
                                <div className="px-6 sm:px-7 pb-6 sm:pb-7 text-zinc-300 text-sm sm:text-base leading-relaxed font-light">
                                    {faq.answer}
                                </div>
                            )}
                        </div>
