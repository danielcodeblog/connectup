...
const FounderDashboard: React.FC<FounderDashboardProps> = React.memo(({ userProfile, onConnect, onEditStateChange, onPostCreationStateChange, onMeetingModalStateChange, isEditingDeck, onNavigateHome }) => {
  // Real Data State
  ...
  return (
    <div className="flex flex-col h-screen w-full relative font-sans text-zinc-900 overflow-hidden">
      {/* Dynamic Background */}
      <div className="absolute inset-0 z-0 bg-[#FFFCF0] pointer-events-none fixed">
      ...
      <div className="relative z-10 flex flex-col h-full overflow-hidden">
          ...
          >
             <div className="py-6 w-full space-y-8 max-w-6xl mx-auto">
                 {/* Pitch Widget - Full Width & Synchronized Height */}
                 <div className="w-full">
                     <div className="bg-white rounded-[32px] shadow-sm border border-zinc-100 backdrop-blur-sm overflow-hidden flex flex-col">
                         <div className="p-5 sm:p-8 flex justify-between items-center shrink-0">
                             <div className="flex items-center gap-4">
                                 <IconWrapper icon={Video01Icon} size={24} className="text-zinc-900" />
                             </div>
                             <button 
                                onClick={() => setIsEditingDeck(true)}
                                className="flex items-center gap-2 px-4 py-2 bg-zinc-900 text-white rounded-full text-xs font-bold hover:bg-zinc-800 transition-colors"
                             >
                                <IconWrapper icon={PencilEdit02Icon} size={14} className="text-white" />
                                Pitch
                             </button>
                         </div>
