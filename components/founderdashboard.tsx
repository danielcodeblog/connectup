  ...
  return (
    <div className="flex flex-col h-full w-full relative overflow-hidden font-sans text-zinc-900">
      ...
      <div className="relative z-10 flex flex-col h-full">
          {/* Header */}
          <div className={`absolute top-0 left-0 right-0 px-4 py-1.5 sm:px-6 sm:py-4 safe-area-top z-20 transition-all duration-200 backdrop-blur-md bg-white/70 border-b border-zinc-100 ${isEditingDeck ? '-translate-y-full opacity-0 pointer-events-none' : 'translate-y-0 opacity-100'}`}>
      ...
      {isMeetingModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 animate-in fade-in duration-300">
            ...
            <div className="relative w-[95vw] max-w-2xl bg-white rounded-[32px] sm:rounded-[48px] shadow-2xl animate-in zoom-in-95 duration-400 overflow-hidden flex flex-col border border-white/20 sm:max-h-[95vh]">
                ...
                {/* Header Section */}
                <div className="px-6 sm:px-8 pt-8 sm:pt-10 pb-4 sm:pb-6 flex justify-between items-start bg-white">
                    <div>
                        <h3 className="font-display font-black text-2xl sm:text-3xl text-zinc-900 tracking-tighter leading-none">Schedule Item</h3>
                    </div>
                    <button 
                        onClick={() => !isSavingMeeting && setIsMeetingModalOpen(false)}
                ...
                {/* Body / Form Section */}
                <div className="px-6 sm:px-8 py-4 sm:py-6 space-y-6 overflow-y-auto max-h-[70vh] no-scrollbar">
                    
                    {/* Event Title */}
                    <div className="space-y-2">
                        <label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest ml-1">What's on the schedule?</label>
                        <input 
                            type="text" 
                            value={meetingForm.title || ''} 
                            onChange={(e) => setMeetingForm(prev => ({ ...prev, title: e.target.value }))} 
                            placeholder="e.g. Call with Investor" 
                            className="w-full bg-zinc-50 border border-zinc-100 rounded-2xl px-6 py-4 text-base font-bold text-zinc-900 placeholder:text-zinc-500 focus:outline-none focus:bg-white transition-all shadow-sm" 
                        />
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="space-y-2">
                            ...
                            <input 
                                ...
                                className="w-full bg-zinc-50 border border-zinc-100 rounded-2xl px-6 py-4 text-sm font-bold text-zinc-900 focus:outline-none focus:bg-white transition-all shadow-sm" 
                            />
                        </div>
                        <div className="space-y-2">
                            <label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest ml-1">Guest Email</label>
                            <input 
                                type="email" 
                ...
                {/* Footer Section */}
                <div className="px-6 sm:px-10 py-8 sm:py-10 bg-zinc-50/80 border-t border-zinc-100 flex gap-4 sm:gap-6">
                    {meetingForm.id && (
                        <div 
                            onClick={handleDeleteMeeting} 
                            className="w-16 h-16 bg-white border border-red-100 text-red-500 rounded-2xl flex items-center justify-center hover:bg-red-50 transition-all shadow-sm cursor-pointer"
                        >
                    ...
                    <div 
                        ...
                        className={`flex-1 h-16 bg-black text-white rounded-2xl font-black text-lg hover:bg-zinc-800 transition-all shadow-xl flex items-center justify-center gap-2 cursor-pointer ${isSavingMeeting || !meetingForm.title || !meetingForm.date ? 'opacity-30 pointer-events-none' : ''}`}
                    >
                        {isSavingMeeting ? (
