...
export const LimelightNav = ({
  ...
  return (
    <nav className={`relative inline-flex items-center justify-between gap-1 h-16 sm:h-20 bg-white/20 dark:bg-black/20 backdrop-blur-2xl border border-white/30 dark:border-white/10 shadow-[0_12px_40px_rgba(0,0,0,0.06)] dark:shadow-[0_12px_40px_rgba(0,0,0,0.4)] rounded-full px-2 sm:px-4 text-zinc-900 dark:text-zinc-100 w-[95%] max-w-lg md:max-w-3xl select-none transition-colors duration-500 ${className}`}>
      {items.map(({ id, icon, label, onClick, badge }, index) => {
        ...
        return (
          <button
            ...
            className={`relative z-20 flex-1 flex h-full items-center justify-center cursor-pointer transition-all duration-300 rounded-full select-none outline-none focus:outline-none ${iconContainerClassName}`}
          ...
          >
            ...
            {isActive && (
              <motion.div
                ...
                className="absolute inset-y-2 sm:inset-y-3 inset-x-1 sm:inset-x-2 bg-zinc-900 dark:bg-zinc-800 rounded-full -z-10 shadow-md border border-zinc-950 dark:border-zinc-700/50"
                transition={{ type: 'spring', stiffness: 400, damping: 30 }}
              />
            )}

            <div className={`relative flex items-center justify-center gap-2 px-3 py-2 sm:px-4 sm:py-2.5 rounded-full transition-all duration-300 ${
              isActive ? 'text-brand-primary' : 'text-zinc-500 dark:text-zinc-400 hover:text-zinc-800 dark:hover:text-zinc-200'
            }`}>
