...
const ChatInterface: React.FC<ChatInterfaceProps> = React.memo(({ 
  ...
  return (
    <div 
      className="flex font-sans text-zinc-900 bg-white fixed lg:relative inset-0 lg:inset-auto h-[var(--vh,100dvh)] lg:h-screen w-full overflow-hidden select-none select-text-for-messages touch-pan-y"
      style={{ overscrollBehaviorY: 'none' }}
    >
      
      {/* --- LEFT SIDEBAR (CHAT LIST) --- */}
      <div className={`
        absolute inset-0 md:relative md:inset-auto md:w-[400px] lg:w-[420px] bg-white border-r border-zinc-200/60 flex flex-col z-20 transition-transform duration-300 md:h-[100dvh] overscroll-contain
        ...
        >
          ...
          <div className="relative group">
            ...
            <input 
              ...
              className="w-full bg-zinc-50/50 border border-zinc-200 rounded-xl py-2 pl-9 pr-3 text-sm font-medium text-zinc-900 placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-brand-primary/20 focus:border-brand-primary focus:bg-white transition-all font-sans"
            />
          </div>
        </div>
      ...
      <div className={`
        absolute inset-0 md:relative md:inset-auto flex flex-col bg-[#F9FAFB] z-30 transition-transform duration-300 md:h-[100dvh] md:flex-1 overflow-hidden
      ...
      `}>
        {selectedChatId ? (
          <>
            {/* Chat Header */}
            <div 
              className="bg-white/95 backdrop-blur-md border-b border-zinc-200/60 px-3.5 py-2.5 sm:px-6 sm:py-4 flex items-center justify-between shrink-0 z-40"
            ...
            >
              ...
              <div className="flex items-center gap-1 text-zinc-400">
                <button 
                    ...
                    className="p-2.5 sm:p-3 hover:bg-zinc-100 rounded-full text-zinc-600 hover:text-[#EAB308] transition-all w-11 h-11 sm:w-12 sm:h-12 flex items-center justify-center active:scale-90 shadow-sm border border-zinc-100 hover:shadow-md cursor-pointer shrink-0"
                            ...
                            <div className={`flex items-end gap-2 ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                              ...
                              <div className={`relative px-3.5 py-2 ${bubbleCorners} shadow-sm text-[14px] sm:text-[15px] leading-relaxed break-words group transition-all select-text ${isMe ? 'bg-[#EAB308] text-black shadow-[#EAB308]/10' : 'bg-white text-zinc-800 border border-zinc-100'}`}>
                                ...
                                {msg.type === 'text' && (
                                   msg.text?.startsWith('http') && (msg.text.includes('giphy') || msg.text.includes('gif')) ? (
                                       <div className="relative">
                                           <div className="absolute top-1 left-1 bg-black/50 text-white text-[10px] px-1 rounded flex items-center gap-0.5">
                                               <GifIcon size={12} /> GIF
                                           </div>
                              ...
                              {msg.type === 'document' && (() => {
                                ...
                                const iconContainerBg = isMe 
                                  ...
                                  : (details.iconName === 'image' ? 'bg-emerald-500/10 text-emerald-600' :
                                     details.iconName === 'spreadsheet' ? 'bg-teal-500/10 text-teal-600' :
                                     details.iconName === 'video' ? 'bg-amber-500/10 text-amber-600' :
                                     details.iconName === 'audio' ? 'bg-amber-500/10 text-amber-600' :
                                     details.iconName === 'pdf' ? 'bg-red-500/10 text-red-600' :
                                     details.iconName === 'archive' ? 'bg-amber-500/10 text-amber-600' : 'bg-amber-500/10 text-amber-600');
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <div className="flex flex-col overflow-hidden flex-1 text-left min-w-0 pr-1">
                                      <span className={`font-bold text-sm truncate max-w-[200px] block transition-colors ${isMe ? 'text-black' : 'text-zinc-800'}`}>{details.name}</span>
                                      <div className="flex items-center gap-1.5 mt-0.5 text-[11px]">
                                        ...
                                        <span className={`truncate text-[11.5px] ${isMe ? 'text-black/60' : 'text-zinc-400'}`}>{msg.fileSize || 'Unknown size'}</span>
                                      </div>
                                    </div>
                                    <div className={`p-2 rounded-full ml-1 shrink-0 ${isMe ? 'hover:bg-black/10 text-zinc-900' : 'hover:bg-zinc-100 text-zinc-500'}`}>
                                      <Download size={16} />
                                    </div>
                                  </a>
                                );
