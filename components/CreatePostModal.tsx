...
export const CreatePostModal: React.FC<CreatePostModalProps> = ({ userProfile, onClose, onPostCreated, quotedPost }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-white flex flex-col animate-in fade-in duration-300">
      <div className="relative w-full h-full max-w-2xl mx-auto bg-white overflow-hidden flex flex-col">
        <div className="px-6 py-6 border-b border-zinc-100 flex justify-between items-center bg-white shrink-0">
          <h3 className="font-bold text-2xl text-zinc-900 font-display">Create Post</h3>
          <button onClick={() => !isPosting && onClose()} className="p-2.5 bg-zinc-50 hover:bg-zinc-100 rounded-full transition-colors text-zinc-400 hover:text-zinc-900">
            <X className="w-5 h-5" />
          </button>
        </div>
