...
import foundersBg from '../src/assets/images/founders.png';
import goldenGateBg from '../src/assets/images/golden_gate_bridge_1785141940553.jpg';
...
export default function LandingSite({ onLoginClick, onAdminLoginClick, onLegalView }: LandingSiteProps) {
  ...
  return (
    <ShaderBackground paused={isMobileMenuOpen} className="font-poppins bg-[#0D0D0F] text-white min-h-screen flex flex-col relative selection:bg-[#FFBF00] selection:text-black overflow-x-hidden max-w-full">
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          ...
          >
            ...
            <div className="flex items-center justify-between mb-4 pb-2 border-b border-white/5 md:mb-12 md:pb-8">
              ...
              >
                <span className="font-[800] text-2xl md:text-3xl tracking-tighter">
                  ...
                  <span className="text-[#FFBF00]">up</span>
                </span>
              </div>
            ...
            >
              {(['home', 'about', 'why', 'founders', 'pricing', 'blog'] as const).map((page) => (
                <button
                  ...
                  className="block w-full text-center text-4xl sm:text-5xl md:text-7xl font-sans font-black text-white hover:text-[#FFBF00] transition-colors tracking-tighter leading-none uppercase"
                >
                  {page}.
                </button>
              ))}
            ...
            <div
              className="pt-10 flex flex-col sm:flex-row items-center justify-center gap-4 w-full pb-12"
            >
              <button
                onClick={() => {
                  setIsMobileMenuOpen(false);
                ...
                className="w-full sm:w-auto px-10 py-5 bg-gradient-to-r from-[#FFBF00] to-[#CFB53B] text-black rounded-full font-black uppercase tracking-[0.2em] text-sm transition-transform shadow-[0_20px_50px_rgba(255,191,0,0.3)] cursor-pointer"
              >
                Get Started
              </button>
            </div>
      ...
      {!isMobileMenuOpen && (
        <nav className="fixed top-0 left-0 right-0 z-[90] px-4 py-4 sm:px-6 pointer-events-none">
          ...
          <div className={`hidden lg:flex max-w-7xl mx-auto h-20 transition-all duration-500 ease-out items-center justify-between px-6 pointer-events-auto ${
            ...
            <div className="hidden lg:flex items-center gap-3 text-nowrap">
               <button 
                onClick={onLoginClick}
                className="px-7 py-2.5 rounded-full font-black text-xs bg-zinc-900 text-white hover:bg-[#FFBF00] hover:text-black transition-all hover:scale-105 active:scale-95 shadow-xl"
               >
                 Login
               </button>
            </div>
      ...
      <main ref={contentRef} className="flex-1 w-full flex flex-col overflow-x-hidden max-w-full">
        ...
        <section 
          id="founders" 
          className="w-full min-h-screen flex items-center justify-center relative z-10 px-4 sm:px-6 py-16 md:py-32 overflow-hidden bg-cover bg-center bg-no-repeat border-t border-amber-300/40"
          style={{ backgroundImage: `url(${foundersBg})` }}
        >
          {/* Subtle Golden Hour Lavender Ambient Glow */}
          <div className="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] rounded-full bg-[#E0B0FF]/25 blur-[120px] pointer-events-none z-0" />
          <div className="max-w-screen-2xl mx-auto w-full relative z-10">
            <FounderSlider founders={reviews} className="rounded-[2.5rem] border-white/40 shadow-[0_20px_50px_rgba(255,191,0,0.25)]" />
          </div>
        </section>
        ...
        {/* Partnering CTA Banner Section */}
        <section className="w-full relative py-28 md:py-36 flex items-center justify-center overflow-hidden bg-zinc-900 border-t border-white/10">
          <div 
            className="absolute inset-0 bg-cover bg-center bg-no-repeat opacity-80 scale-105 transition-transform duration-1000"
            style={{ 
              backgroundImage: `url(${goldenGateBg})` 
            }}
          />
          ...
          <div className="max-w-5xl mx-auto px-6 text-center relative z-10 space-y-8">
            <h2 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-display font-medium text-white tracking-tight leading-tight drop-shadow-md">
              Build what's next.
            </h2>
            
            <div className="max-w-2xl mx-auto pt-4 flex items-center justify-center">
              <form 
                onSubmit={(e) => {
                  e.preventDefault();
                  ...
                  if (text) {
                    const mailtoUrl = `mailto:support@connectupng.com?subject=${encodeURIComponent("Concern Submission")}&body=${encodeURIComponent(text)}`;
                    window.location.href = mailtoUrl;
                    input.value = '';
                  } else {
                    window.location.href = "mailto:support@connectupng.com";
                  }
                }} 
                className="w-full flex flex-col sm:flex-row items-center gap-2 bg-white/10 backdrop-blur-md p-2 rounded-2xl sm:rounded-full border border-white/30 shadow-2xl"
              >
                <input
                  type="text"
                  placeholder="Send a concern..."
                  className="w-full sm:flex-1 bg-transparent text-white placeholder-white/70 px-5 py-3 outline-none text-sm font-medium"
                />
                <button
                  type="submit"
                  className="w-full sm:w-auto px-6 py-3 bg-white text-zinc-900 hover:bg-zinc-100 font-bold rounded-xl sm:rounded-full shadow-lg transition-all transform hover:-translate-y-0.5 active:translate-y-0 whitespace-nowrap cursor-pointer text-sm"
                >
                  Send concern
                </button>
              </form>
