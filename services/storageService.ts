...
let _isMock = false;
let _meetingsSupportsComplexQuery = true;

let _cachedUser: any = null;
let _cachedUserPromise: Promise<any> | null = null;

const getAuthenticatedUser = async () => {
    if (_isMock) return { id: 'mock-user-123', email: 'mock@example.com' };
...
const handleSupabaseError = (e: any, methodName: string) => {
    if (!e) return false;
    if (e.code === '22P02' || e.code === '42501' || e.code === 'PGRST116') {
        console.warn(`Supabase notice [${e.code}] in ${methodName}:`, e?.message || e);
        return true;
    }
...
export const StorageService = {
  ...
  init: (): Promise<StorageInitStatus> => {
    ...
    initPromise = (async () => {
        ...
        try {
            ...
            console.log("Supabase connection check result:", profileError ? profileError.message : "Success");
            ...
            // Initialize encryption in the background
            StorageService.ensureEncryptionInitialized().catch(e => console.error(e));
  ...
  getStartups: async (): Promise<Startup[]> => {
      ...
      try {
          ...
          if (user && isUUID(user.id)) {
              ...
              if (swipes && swipes.length > 0) swipedIds = swipes.map((s: any) => s.startup_id).filter(id => typeof id === 'string' && id.length > 0);
          }
          let query = supabase.from('startups').select('*').order('created_at', { ascending: false }).limit(20);
          const validUuidSwipedIds = swipedIds.filter(id => isUUID(id));
          if (validUuidSwipedIds.length > 0) {
              query = query.not('id', 'in', validUuidSwipedIds);
          }
          const { data: startupData, error: startupError } = await query;
          if (startupError || !startupData || startupData.length === 0) {
              return MOCK_STARTUPS.filter(s => !swipedIds.includes(s.id));
          }
          const userIds = startupData.map((s: any) => s.id);
          if (userIds.length === 0) {
              return MOCK_STARTUPS.filter(s => !swipedIds.includes(s.id));
          }
          const [profileResult, metricsResult, pitchesResult] = await Promise.all([
          ...
          for (const s of startupData) {
              ...
              const baseStartup = {
                  ...
                  socialMediaUrl: s.social_media_url || '',
                  fundingHistory: s.funding_history || [],
              ...
              if (userPitches && userPitches.length > 0) {
                  ...
                  for (let i = userPitches.length - 1; i >= 0; i--) {
                      ...
                      startups.push({
                          ...
                          socialMediaUrl: p.social_media_url || baseStartup.socialMediaUrl,
                          fundingHistory: p.funding_history || baseStartup.fundingHistory
                      });
                  }
              } else {
  ...
  getMyPitches: async (): Promise<Startup[]> => {
      await StorageService.init();
      try {
          const user = await getAuthenticatedUser();
          ...
          if (!_isMock && user?.id) {
              try {
                  ...
                  if (!error && data && data.length > 0) {
                      ...
                      const pitchesFromDb: Startup[] = data.map((row: any) => ({
                          id: row.id,
                          name: row.name || 'Startup Pitch',
                          oneLiner: row.one_liner || '',
                          description: row.description || '',
                          industry: row.industry || '',
                          fundingStage: row.funding_stage || '',
                          askAmount: row.ask_amount || 0,
                          valuationCap: row.valuation_cap || 0,
                      ...
                      localStorage.setItem(`connectup_pitches_${uid}`, JSON.stringify(pitchesFromDb));
                      return pitchesFromDb;
                  }
              } catch (e) {
                  console.warn("Error loading pitches from Supabase:", e);
              }
          }

          const cached = localStorage.getItem(`connectup_pitches_${uid}`);
          if (cached !== null) {
              try {
          ...
          const single = await StorageService.getMyStartup();
          if (single) {
              const arr = [single];
              localStorage.setItem(`connectup_pitches_${uid}`, JSON.stringify(arr));
              return arr;
          }
          return [];
      } catch (e) {
  ...
  savePitch: async (startup: Startup, pitchIndex?: number): Promise<Startup[]> => {
      await StorageService.init();
      try {
          const user = await getAuthenticatedUser();
          const uid = user?.id || 'guest';
          const pitches = await StorageService.getMyPitches();
          
          let updatedPitches = [...pitches];
          if (pitchIndex !== undefined && pitchIndex >= 0 && pitchIndex < updatedPitches.length) {
              updatedPitches[pitchIndex] = { ...updatedPitches[pitchIndex], ...startup };
          } else {
              const existingIdx = startup.id ? updatedPitches.findIndex(p => p.id === startup.id) : -1;
              if (existingIdx >= 0) {
                  updatedPitches[existingIdx] = { ...updatedPitches[existingIdx], ...startup };
              } else {
                  if (!startup.id) startup.id = generateUUID();
                  updatedPitches.push(startup);
              }
          }
          
          localStorage.setItem(`connectup_pitches_${uid}`, JSON.stringify(updatedPitches));

          if (!_isMock && user?.id) {
              try {
                  const targetPitch = pitchIndex !== undefined && pitchIndex >= 0 && pitchIndex < updatedPitches.length
                      ? updatedPitches[pitchIndex]
                      : updatedPitches[updatedPitches.length - 1];

                  const pitchPayload: any = {
                      user_id: user.id,
                      name: targetPitch.name || 'Startup Pitch',
                      one_liner: targetPitch.oneLiner || '',
                      description: targetPitch.description || '',
                      industry: targetPitch.industry || '',
                      funding_stage: targetPitch.fundingStage || '',
                      ask_amount: targetPitch.askAmount || 0,
                      valuation_cap: targetPitch.valuationCap || 0,
                      image_url: targetPitch.imageUrl || '',
                      video_url: targetPitch.videoUrl || '',
                      social_media_url: targetPitch.socialMediaUrl || '',
                      funding_history: targetPitch.fundingHistory || [],
                      tags: targetPitch.tags || [],
                      is_active: true
                  };

                  if (targetPitch.id && targetPitch.id.includes('-')) {
                      pitchPayload.id = targetPitch.id;
                  }
                  ...
                  if (!error && data) {
                      ...
                      if (pitchIndex !== undefined && pitchIndex >= 0 && pitchIndex < updatedPitches.length) {
                          updatedPitches[pitchIndex].id = finalId;
                      } else {
                          updatedPitches[updatedPitches.length - 1].id = finalId;
                      }
                      localStorage.setItem(`connectup_pitches_${uid}`, JSON.stringify(updatedPitches));
                  }
              } catch (e) {
                  console.warn("Could not save pitch to DB:", e);
              }
          }
          
          if (updatedPitches.length > 0) {
              try {
                  await StorageService.saveStartup(updatedPitches[0]);
              } catch (e) {
                  console.warn("Could not sync primary pitch to DB:", e);
              }
          }
          return updatedPitches;
      } catch (e) {
          console.error("savePitch error:", e);
          return [];
      }
  },

  deletePitch: async (pitchIdOrIndex: string | number): Promise<Startup[]> => {
      await StorageService.init();
      try {
          const user = await getAuthenticatedUser();
          const uid = user?.id || 'guest';
          let pitches = await StorageService.getMyPitches();
          let deletedPitchId: string | null = null;

          if (typeof pitchIdOrIndex === 'number') {
              if (pitchIdOrIndex >= 0 && pitchIdOrIndex < pitches.length) {
                  deletedPitchId = pitches[pitchIdOrIndex].id || null;
              }
              pitches = pitches.filter((_, idx) => idx !== pitchIdOrIndex);
          } else {
              deletedPitchId = pitchIdOrIndex;
              pitches = pitches.filter(p => p.id !== pitchIdOrIndex);
          }

          localStorage.setItem(`connectup_pitches_${uid}`, JSON.stringify(pitches));

          if (!_isMock && user?.id && deletedPitchId && deletedPitchId.includes('-')) {
              try {
                  await supabase
                      .from('pitches')
                      .delete()
                      .eq('id', deletedPitchId)
                      .eq('user_id', user.id);
              } catch (e) {
                  console.warn("Could not delete pitch from DB:", e);
              }
          }
  ...
  getCommunityTopics: async (): Promise<string[]> => {
      await StorageService.init();
      if (!_isMock) {
          try {
              const { data, error } = await supabase.from('community_topics').select('name').order('created_at', { ascending: true });
              if (!error && data) {
                  const dbTopics = data.map((t: any) => t.name);
                  localStorage.setItem('connectup_community_topics', JSON.stringify(dbTopics));
                  return dbTopics;
              }
          } catch (e) {
              console.warn("Could not fetch community topics from DB:", e);
          }
      }
      try {
          const saved = localStorage.getItem('connectup_community_topics');
          if (saved) return JSON.parse(saved);
      } catch (e) {}
      return [];
  },

  addCommunityTopic: async (topicName: string): Promise<string[]> => {
      await StorageService.init();
      const clean = topicName.trim().replace(/^#/, '');
      if (!clean) return await StorageService.getCommunityTopics();
      ...
      if (!_isMock) {
          try {
              ...
              console.warn("Could not insert topic to DB:", e);
          }
      }

      const current = await StorageService.getCommunityTopics();
      if (!current.some(t => t.toLowerCase() === clean.toLowerCase())) {
          const updated = [...current, clean];
          localStorage.setItem('connectup_community_topics', JSON.stringify(updated));
          return updated;
      }
      return current;
  },

  deleteCommunityTopic: async (topicName: string): Promise<string[]> => {
      await StorageService.init();
      const clean = topicName.trim().replace(/^#/, '');
      if (!_isMock) {
          try {
              await supabase.from('community_topics').delete().ilike('name', clean);
          } catch (e) {
              console.warn("Could not delete topic from DB:", e);
          }
      }
      const current = await StorageService.getCommunityTopics();
  ...
  saveStartup: async (startup: Startup) => {
      ...
      try {
          ...
          const payload = {
               ...
               social_media_url: startup.socialMediaUrl || '',
               funding_history: startup.fundingHistory || [],
               tags: startup.tags, 
          };
  ...
  processSwipe: async (startupId: string, direction: 'left' | 'right') => {
      ...
      // Handle mock or non-UUID startup IDs
      if (_isMock || !isUUID(startupId)) {
      ...
      try {
          ...
          // Attempt insert first to avoid RLS update-policy checks on new rows
          ...
          if (insertError) {
              if (insertError.code === '23505') {
                  await supabase.from('swipes').update({
                      direction: direction,
                      created_at: new Date().toISOString()
                  }).eq('user_id', user.id).eq('startup_id', startupId);
              }
          }
      } catch (e: any) { 
          // Silent catch for smooth swipe execution
      }
  },
  ...
  getCommunityPosts: async (page: number = 0, limit: number = 10, sortBy: 'recent' | 'liked' = 'recent', tag?: string | null): Promise<CommunityPost[]> => {
      ...
      try {
          ...
          // Fetch current user's likes, follows, and bookmarks if logged in
          let userLikes = new Set<string>();
          let userFollows = new Set<string>();
          let userBookmarks = new Set<string>();
          
          let cachedBookmarks: Record<string, boolean> = {};
          try {
              cachedBookmarks = JSON.parse(localStorage.getItem('connectup_bookmarks') || '{}');
          } catch (e) {}
          
          if (user) {
              const [likesRes, followsRes, bookmarksRes] = await Promise.all([
                  supabase.from('community_likes').select('post_id').eq('user_id', user.id),
                  supabase.from('follows').select('following_id').eq('follower_id', user.id),
              ...
              if (likesRes.data) userLikes = new Set(likesRes.data.map((l: any) => l.post_id));
              if (followsRes.data) userFollows = new Set(followsRes.data.map((f: any) => f.following_id));
              if (bookmarksRes.data) userBookmarks = new Set(bookmarksRes.data.map((b: any) => b.post_id));
          }
          ...
          return posts.map((p: any) => {
              ...
              return {
                  ...
                  time: p.created_at ? new Date(p.created_at).toLocaleDateString() : 'Recently',
                  isLiked: userLikes.has(p.id),
                  isBookmarked: userBookmarks.has(p.id) || !!cachedBookmarks[p.id],
  ...
  createCommunityPost: async (content: string, tags: string[], author: any, image?: File, quotedPostId?: string) => {
      ...
      // Extract all hashtags from content and add as topics
      const extractedTags = Array.from(content.matchAll(/#([\w-]+)/g)).map(m => m[1]);
      const allTags = Array.from(new Set([...(tags || []), ...extractedTags]));
      for (const t of allTags) {
          await StorageService.addCommunityTopic(t);
      }
      if (_isMock) {
  ...
  addCommentToPost: async (postId: string, content: string, author: any): Promise<{ success: boolean, comment?: CommunityComment, error?: string }> => {
      ...
      // Extract all hashtags from comment content and add as topics
      const commentTags = Array.from(content.matchAll(/#([\w-]+)/g)).map(m => m[1]);
  ...
  toggleBookmarkPost: async (postId: string, isBookmarked: boolean): Promise<boolean> => {
      await StorageService.init();
      try {
          const cached = JSON.parse(localStorage.getItem('connectup_bookmarks') || '{}');
          if (isBookmarked) {
              cached[postId] = true;
          } else {
              delete cached[postId];
          }
          localStorage.setItem('connectup_bookmarks', JSON.stringify(cached));
      } catch (e) {
          console.warn("Error updating local bookmarks cache:", e);
      }
      ...
      try {
          ...
          if (isBookmarked) {
              const { error } = await supabase.from('community_bookmarks').insert({ post_id: postId, user_id: user.id });
              if (error && error.code !== '23505') throw error;
          } else {
              const { error } = await supabase.from('community_bookmarks').delete().eq('post_id', postId).eq('user_id', user.id);
              if (error) throw error;
          }
          return true;
      } catch (e) {
          console.error("Error in toggleBookmarkPost:", e);
          return true;
      }
  },

  getBookmarkedPosts: async (): Promise<CommunityPost[]> => {
      await StorageService.init();
      let cachedBookmarks: Record<string, boolean> = {};
      try {
          cachedBookmarks = JSON.parse(localStorage.getItem('connectup_bookmarks') || '{}');
      ...
      if (_isMock) {
          return MOCK_COMMUNITY_POSTS.filter(p => p.isBookmarked || cachedBookmarks[p.id]);
      }
      ...
      try {
          ...
          if (user) {
              const { data: bmData } = await supabase
                  ...
                  .eq('user_id', user.id);
              if (bmData) {
                  bookmarkedPostIds = bmData.map((b: any) => b.post_id);
              }
          }

          const localIds = Object.keys(cachedBookmarks).filter(id => cachedBookmarks[id]);
          const allBookmarkedIds = Array.from(new Set([...bookmarkedPostIds, ...localIds]));

          if (allBookmarkedIds.length === 0) return [];
          ...
          const authorIds = [...new Set(posts.map((p: any) => p.author_id || p.user_id || p.profile_id).filter(Boolean))];
          const { data: profiles } = await supabase.from('profiles').select('*').in('id', authorIds);
          const profileMap = new Map<string, any>(profiles?.map((p: any) => [p.id, p]) || []);

          let userLikes = new Set<string>();
          if (user) {
              const likesRes = await supabase.from('community_likes').select('post_id').eq('user_id', user.id);
              if (likesRes.data) userLikes = new Set(likesRes.data.map((l: any) => l.post_id));
          }
          ...
          console.error("Error in getBookmarkedPosts:", e);
          return [];
      }
  },
  ...
  getMeetings: async (): Promise<Meeting[]> => {
      ...
      try {
          ...
          if (_meetingsSupportsComplexQuery) {
              const { data: complexData, error } = await supabase
                  .from('meetings')
                  .select('*')
                  .or(`investor_id.eq.${user.id},startup_id.eq.${user.id},user_id.eq.${user.id}`);
              
              if (error) {
                  // If the columns investor_id or startup_id don't exist, turn off complex queries permanently for this session
                  if (error.message?.includes('column') && (error.message?.includes('investor_id') || error.message?.includes('startup_id'))) {
                      _meetingsSupportsComplexQuery = false;
                      console.info("Meetings table does not support multi-role queries (investor_id/startup_id columns are missing). Falling back silently to user_id filter.");
                  } else {
                      console.warn("Meeting complex query failed:", error.message);
                  }
  ...
  ensureEncryptionInitialized: async (): Promise<void> => {
      if (_isMock) return;
      try {
          const user = await getAuthenticatedUser();
          if (user) {
              await EncryptionService.init(user.id);
              const myPublicKey = EncryptionService.getMyPublicKey();
              // if (myPublicKey) {
              //     const { data: profile } = await supabase.from('profiles').select('id').eq('id', user.id).maybeSingle();
              //     if (profile && !profile.encryption_public_key) {
              //         await supabase.from('profiles').update({ encryption_public_key: myPublicKey }).eq('id', user.id);
              //     }
              // }
          }
      } catch (e) {
  ...
  getPeerPublicKeyForChat: async (chatId: string): Promise<string | null> => {
      await StorageService.init();
      return null;
  },
  ...
  getChats: async (): Promise<ChatSession[]> => {
      ...
      try {
          ...
          let profilesData: any[] = profilesResult.data || [];
          if (profilesResult.error) {
              handleSupabaseError(profilesResult.error, 'getChats-profiles');
              profilesData = [];
          }
          ...
          return await Promise.all(rawChats.map(async (c: any) => {
              ...
              const peerKey = partnerProfile?.encryption_public_key || null;
  ...
  getMessages: async (chatId: string, peerPublicKey?: string): Promise<Message[]> => {
      await StorageService.init();
      await StorageService.ensureEncryptionInitialized();
      if (_isMock) {
      ...
      try {
          ...
          //     if (chat) {
          //         const partnerId = chat.investor_id === user.id ? chat.startup_id : chat.investor_id;
          //         const { data: partner } = await supabase.from('profiles').select('encryption_public_key').eq('id', partnerId).maybeSingle();
          //         peerKey = partner?.encryption_public_key || null;
          //     }
          // }
  ...
  addMessage: async (message: Message, chatId: string, peerPublicKey?: string): Promise<Message | null> => {
      ...
      try {
          ...
          if (!peerKey) {
              const { data: chat } = await supabase.from('chats').select('startup_id, investor_id').eq('id', chatId).maybeSingle();
              if (chat) {
                  const partnerId = chat.investor_id === userId ? chat.startup_id : chat.investor_id;
                  // const { data: partner } = await supabase.from('profiles').select('encryption_public_key').eq('id', partnerId).maybeSingle();
                  // peerKey = partner?.encryption_public_key || null;
              }
          }
  ...
  submitPostReport: async (target: any, reason: string): Promise<boolean> => {
      const postId = typeof target === 'string' ? target : target?.id;
      const authorId = typeof target === 'object' ? (target?.authorId || target?.user_id || target?.userId) : null;
      const content = typeof target === 'object' ? target?.content : null;
      const videoUrl = typeof target === 'object' ? target?.videoUrl : null;
      ...
      try {
          const user = await getAuthenticatedUser();
          if (!user) {
              console.warn("No authenticated user found for report submission");
              return false;
          }
          ...
          const payload: any = {
              reporter_id: user.id,
              reason: videoUrl ? `${reason} [VIDEO:${videoUrl}]` : reason,
              target_type: 'post',
              target_id: isValidUUID(postId) ? postId : null,
              target_content: content || (videoUrl ? '[Video Post]' : null)
          };
          
          if (authorId && isValidUUID(authorId)) {
              payload.reported_profile_id = authorId;
          } else if (postId && isValidUUID(postId)) {
              payload.reported_profile_id = postId;
          } else {
              payload.reported_profile_id = user.id; // Emergency fallback
          }

          console.log("Submitting report payload (Simplified):", payload);
          const { error } = await supabase.from('reports').insert(payload);
          
          if (error) {
              console.error("Supabase error submitting report:", JSON.stringify(error, null, 2));
              // Final fallback
              if (error.code === '23502') {
                  ...
                  const { error: retryError } = await supabase.from('reports').insert(minimalPayload);
                  return !retryError;
              }
              return false;
          }
          ...
          console.error("Error submitting post report:", e);
          return false;
      }
  },

  submitReport: async (target: any, reason: string): Promise<boolean> => {
      let startupId = typeof target === 'string' ? target : target?.id;
      if (startupId && typeof startupId === 'string') startupId = startupId.split('-pitch')[0];
      
      const authorId = typeof target === 'object' ? (target?.userId || target?.user_id || target?.authorId) : null;
      const startupName = typeof target === 'object' ? target?.name : null;
      const startupOneLiner = typeof target === 'object' ? target?.oneLiner : null;
      const videoUrl = typeof target === 'object' ? target?.videoUrl : null;
      ...
      try {
          ...
          const payload: any = {
              ...
              reason: videoUrl ? `${reason} [VIDEO:${videoUrl}]` : reason,
              target_type: 'startup',
              target_id: isValidUUID(startupId) ? startupId : null,
              target_content: startupName ? `${startupName}: ${startupOneLiner || ''}` : null
          };
          
          if (authorId && isValidUUID(authorId)) {
              payload.reported_profile_id = authorId;
          } else if (startupId && isValidUUID(startupId)) {
              payload.reported_profile_id = startupId;
          } else {
              payload.reported_profile_id = user.id; // Emergency fallback
          }

          console.log("Submitting startup report payload (Simplified):", payload);
          const { error } = await supabase.from('reports').insert(payload);
          
          if (error) {
              console.error("Supabase error submitting startup report:", JSON.stringify(error, null, 2));
              // Final fallback
              if (error.code === '23502') {
                  const minimalPayload = {
                      reporter_id: user.id,
  ...
  getAdminSignupDisabled: (): boolean => {
      try {
          return localStorage.getItem('connectup_admin_signup_disabled') === 'true';
      } catch {
          return false;
      }
  },

  setAdminSignupDisabled: (disabled: boolean): void => {
      try {
          localStorage.setItem('connectup_admin_signup_disabled', disabled ? 'true' : 'false');
      } catch (e) {
          console.error("Error setting admin signup lock:", e);
      }
  }
};