  ...
  addMessage: async (message: Message, chatId: string): Promise<Message | null> => {
      ...
      try {
          ...
          if (error && error.message.includes('column') && error.message.includes('status')) {
              console.warn("Retrying message insert without status column");
              const safePayload = { ...payload };
              delete safePayload.status;
              const retry = await supabase.from('messages').insert(safePayload).select().single();
              data = retry.data;
              error = retry.error;
          }
          ...
          const { error: chatUpdateError } = await supabase.from('chats').update({ updated_at: data.created_at }).eq('id', chatId);
          if (chatUpdateError) {
          ...
          (async () => {
              const { data: chat } = await supabase.from('chats').select('startup_id, investor_id').eq('id', chatId).single();
              if (chat) {
                  const recipientId = chat.startup_id === userId ? chat.investor_id : chat.startup_id;
                  const { data: profile } = await supabase.from('profiles').select('email').eq('id', recipientId).single();
                  if (profile?.email) {
