...
-- Note: Sensitive fields like 'email' should be managed carefully by the client
-- unless we move them to a separate table or use a view.
-- For this app, we stick to a simple public profile.
...
CREATE INDEX IF NOT EXISTS idx_cm_posts_author ON public.community_posts(author_id);
CREATE INDEX IF NOT EXISTS idx_cm_posts_created ON public.community_posts(created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_cm_comments_post ON public.community_comments(post_id);
...
CREATE INDEX IF NOT EXISTS idx_cm_bookmarks_user ON public.community_bookmarks(user_id);
CREATE INDEX IF NOT EXISTS idx_cm_bookmarks_post ON public.community_bookmarks(post_id);
...
CREATE POLICY "Bookmarks are viewable by their owner" ON public.community_bookmarks FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Authenticated users can bookmark" ON public.community_bookmarks FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can remove bookmarks" ON public.community_bookmarks FOR DELETE USING (auth.uid() = user_id);
...
BEGIN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='startups' AND column_name='social_media_url') THEN
        ALTER TABLE public.startups ADD COLUMN social_media_url TEXT;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='startups' AND column_name='funding_history') THEN
        ALTER TABLE public.startups ADD COLUMN funding_history JSONB DEFAULT '[]'::jsonb;
    END IF;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='reports' AND column_name='target_type') THEN
        ALTER TABLE public.reports ADD COLUMN target_type TEXT DEFAULT 'user';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='reports' AND column_name='target_id') THEN
        ALTER TABLE public.reports ADD COLUMN target_id UUID;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='reports' AND column_name='target_content') THEN
        ALTER TABLE public.reports ADD COLUMN target_content TEXT;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='reports' AND column_name='severity') THEN
        ALTER TABLE public.reports ADD COLUMN severity TEXT DEFAULT 'medium';
    END IF;
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='reports' AND column_name='notes') THEN
...
CREATE POLICY "Users can update their own swipes" ON public.swipes FOR UPDATE USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
CREATE INDEX IF NOT EXISTS idx_chats_startup ON public.chats(startup_id);
CREATE INDEX IF NOT EXISTS idx_chats_investor ON public.chats(investor_id);
...
-- Essential Performance Indexes for Large Scale (100k+ users)
...
CREATE INDEX IF NOT EXISTS idx_meetings_startup ON public.meetings(startup_id);
CREATE INDEX IF NOT EXISTS idx_meetings_investor ON public.meetings(investor_id);
...
-- Pitches Table (for multi-pitch management per founder)
...
CREATE POLICY "Users can insert their own pitches" ON public.pitches FOR INSERT WITH CHECK (auth.uid() = user_id);
...
-- Community Topics Table (for community discussion topics)
...
CREATE INDEX IF NOT EXISTS idx_community_topics_name ON public.community_topics(name);

ALTER TABLE public.community_topics ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Topics are viewable by everyone" ON public.community_topics FOR SELECT USING (true);
CREATE POLICY "Authenticated users can create topics" ON public.community_topics FOR INSERT WITH CHECK (auth.role() = 'authenticated');
CREATE POLICY "Topic creators can delete topics" ON public.community_topics FOR DELETE USING (auth.uid() = created_by);

-- Topic column on community posts
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='community_posts' AND column_name='topic') THEN
        ALTER TABLE public.community_posts ADD COLUMN topic TEXT;
        CREATE INDEX IF NOT EXISTS idx_cm_posts_topic ON public.community_posts(topic);
    END IF;
END $$;
