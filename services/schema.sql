...
begin
  ...
  if (exists (select 1 from information_schema.tables where table_schema = 'public' and table_name = 'community_posts')) then
    if not exists (select 1 from information_schema.columns where table_name = 'community_posts' and column_name = 'quoted_post_id') then
      alter table public.community_posts add column quoted_post_id uuid references public.community_posts(id) on delete set null;
    end if;
    if not exists (select 1 from information_schema.columns where table_name = 'community_posts' and column_name = 'reposts') then
      alter table public.community_posts add column reposts int default 0;
    end if;
  end if;
...
create policy "Users can insert their own reports" on public.reports for insert with check ((select auth.uid()) = reporter_id);
...
-- Add performance indexes for large scale
