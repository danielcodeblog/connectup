...
CREATE POLICY "Users can insert meetings" ON public.meetings FOR INSERT WITH CHECK (auth.role() = 'authenticated');
...
CREATE POLICY "Users can delete their meetings" ON public.meetings FOR DELETE USING (auth.uid() = investor_id OR auth.uid() = startup_id);
