...
const App = () => {
  const [appState, setAppState] = useState<AppState>(() => {
    ...
    const isAuthRequested = pathname === '/auth' || hash === '#auth';
    if (isAdminRequested) {
      if (isLoggedIn) {
        return 'MAIN_APP';
      }
      return 'ADMIN_AUTH' as any;
    }
    if (isAuthRequested) {
      if (isLoggedIn) {
        return 'MAIN_APP';
  ...
  // Helper to resolve canonical URL path for state, view & role
  const getCanonicalPath = useCallback((state: AppState | string, view: string, userRole: UserRole) => {
    if (state === 'LANDING') return '/home';
    if (state === 'AUTH') return '/auth';
    if (state === 'ADMIN_AUTH') return '/admin';
    if (state === 'MAIN_APP') {
      ...
      if (view === 'home') {
        ...
        if (userRole === UserRole.INVESTOR) return '/investor';
        return '/home';
      }
    }
    return '/home';
  ...
  // Sync browser URL location state with appState, currentView, and user role
  useEffect(() => {
    if (appState === 'INITIALIZING' || appState === 'SPLASH') return;

    const targetPath = getCanonicalPath(appState, currentView, role);
    const currentPath = window.location.pathname.toLowerCase();

    if (currentPath !== targetPath && currentPath !== targetPath + '/') {
      try {
        window.history.pushState({ appState, currentView, role }, '', targetPath);
      } catch (e) {
        console.warn('History pushState error:', e);
      }
    }
  }, [appState, currentView, role, getCanonicalPath]);

  // Listen for browser navigation events (Back/Forward & hash changes)
  useEffect(() => {
    const handleUrlRouteSync = () => {
      ...
      if (isAdminRequested) {
        if (isLoggedIn) {
          ...
          setAppState('ADMIN_AUTH' as any);
        }
        return;
      }

      if (pathname === '/auth' || hash === '#auth') {
        if (!isLoggedIn) {
          setAppState('AUTH');
        } else {
          setAppState('MAIN_APP');
      ...
      if (pathname === '/founders' || pathname === '/founder') {
        setRole(UserRole.FOUNDER);
        if (isLoggedIn) {
          setAppState('MAIN_APP');
          setCurrentView('home');
        } else {
      ...
      if (pathname === '/investor' || pathname === '/investors') {
  ...
  const handleAuthComplete = useCallback(async (selectedRole: UserRole, email?: string) => {
    ...
    localStorage.setItem('connectup_logged_in', 'true');
    setAppState('MAIN_APP');
    const targetView = selectedRole === UserRole.ADMIN ? 'admin' : 'home';
    setCurrentView(targetView);
    localStorage.setItem('connectup_current_view', targetView);
  }, []);
  ...
  const mainContent = useMemo(() => {
    ...
    if (currentView === 'admin') {
      ...
      const isAdmin = role === UserRole.ADMIN || MASTER_ADMIN_EMAILS.includes(userEmail);

      if (!isAdmin) {
        return (
          <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-white dark:bg-zinc-950 text-center font-sans">
            <div className="w-16 h-16 bg-red-100 dark:bg-red-950/50 text-red-600 rounded-full flex items-center justify-center mb-4 border border-red-200 dark:border-red-900/50 shadow-inner">
              <Shield size={32} />
            </div>
            ...
            <button 
              ...
              className="bg-zinc-900 text-white dark:bg-amber-400 dark:text-zinc-950 px-8 py-3.5 rounded-full font-extrabold text-sm transition-all shadow-md hover:scale-[1.02] active:scale-[0.98] cursor-pointer"
            >
  ...
  return (
    <div className={`${appState === 'LANDING' || appState === 'AUTH' || appState === 'SPLASH' || appState === 'INITIALIZING' ? 'bg-white dark:bg-[#0D0D0F]' : 'bg-[#FFFCF0] dark:bg-[#0D0D0F]'} flex selection:bg-brand-primary/20 selection:text-brand-primary ${appState === 'LANDING' ? 'min-h-screen' : isLockedView ? 'h-[100dvh] w-full overflow-hidden overscroll-none' : 'min-h-screen w-full relative'} relative transition-colors duration-700 font-sans`}>
      {appState !== 'LANDING' && appState !== 'AUTH' && appState !== 'SPLASH' && appState !== 'INITIALIZING' && currentView !== 'admin' && (
        <>
        ...
        {appState === 'INITIALIZING' && StorageService.isMockMode() === false ? (
          ...
          <>
            <LandingSite 
              ...
              onLegalView={setGlobalLegalView} 
            />
            {globalLegalView && (
          ...
          <>
            <AuthScreen 
              ...
              onComplete={handleAuthComplete} 
              onViewLegal={setGlobalLegalView} 
              onBackHome={() => setAppState('LANDING')} 
            />
            {globalLegalView && (
              <LegalView type={globalLegalView} onBack={() => setGlobalLegalView(null)} />
            )}
